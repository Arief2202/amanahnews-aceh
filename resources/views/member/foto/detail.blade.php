        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-dark2 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="row mb-4">
                        <div class="col-auto">
                            <div class="input-group">
                                ...
                                @if($post->show == 1) <a href="{{route('member.foto.publish', ['id' => $post->id])}}" class="btn btn-success disabled" disabled>Published</a>
                                @elseif($post->show == 0) <a href="{{route('member.foto.unpublish', ['id' => $post->id])}}" class="btn btn-danger disabled" disabled>Not Publish</a>
                                @endif
                            </div>
                        </div>
                        <div class="col">
                            @if($post->show == 0) <a href="{{route('member.foto.publish', ['id' => $post->id])}}?source=detail" class="btn btn-success">Publish</a>
                            @elseif($post->show == 1) <a href="{{route('member.foto.unpublish', ['id' => $post->id])}}?source=detail" class="btn btn-secondary">Unpublish</a>
                            @endif
                        </div>
                        <div class="col d-flex justify-content-end">
                            <a href="{{route('member.foto.delete', ['id' => $post->id])}}" class="btn btn-danger me-3">Delete Post</a>
                            <a href="{{route('member.foto.update', ['id' => $post->id])}}" class="btn btn-warning">Edit Post</a>
                        </div>
                    </div>
                    <div class="row mb-4">
                        <div class="col-md-3">
                    ...
                    <hr>
                    <div class="mt-3">
                        Link Post : <a href="{{route('foto.detail', ['slug' => $post->slug])}}">{{route('foto.detail', ['slug' => $post->slug])}}</a>
                    </div>
                    <div class="ps-3">                
                        <div class="mt-3 mb-2">
                                ...
                                <form action="{{route('postcontent.save')}}" method="POST" enctype="multipart/form-data"> @csrf
                                    ...
                                    <hr>
                                        ...
                                        <div class="mb-5 img-preview-div" style="@if($pc->content == null) display:none;@endif">                            
                                            ...
                                            <img for="image" src="/uploads/foto/image/{{$pc->content}}" alt="" class="img-preview img-fluid" style="display:hidden; max-width:200px; max-height:150px;">
                                        </div>                                        
                                        <div class="row">
                                            <div class="col-md">
                                                <div class="mb-3">
                            ...
                            @else
                                ...
                                <div class="row p-0 m-0">
                                    {{-- <div class="col-xl-8" style=""> --}}
                                        ...
                                        <img src="/uploads/foto/image/{{$pc->content}}" alt="" style="max-width:{{$pc->image_width}}px; max-height:{{$pc->image_height}}px;">
                                        @if($pc->href)</a>@endif
    ...
    <x-slot name="script">
        <script type="text/javascript">     
            ...
            function deleteTag(id){
                ...
                fetch("{{route('member.foto.tag.delete')}}?id="+id)
                .then(response => response.json())
                .then(data => {
                    console.log(data);
                });
