        ...
        <div class="" style="height: 500px">
            ...
            <div class="topBar">
                <h2
                    ...
                    E-Katalog</h2>
                <p style="color:white;font-size:20px; margin:0px;padding:0px;text-align:center; margin-bottom: 50px;">Dukung
                    UMKM dengan membeli produk dari kami. Sebagian besar produk merupakan hasil kolaborasi antara Amanah dan
                    Pemuda Setempat</p>
            </div>
        </div>
    </section>
                            ...
                            <a href="{{ route('e-catalog.detail', ['slug' => $ecatalog->slug]) }}">
                                <div class="shadow" style="border-radius: 15px">
                                    ...
                                    <div class="p-3">
                                        ...
                                        <a href="{{ route('e-catalog.detail', ['slug' => $ecatalog->slug]) }}"
                                            class="btn btn-primary-orange" style="width:100%; border-radius:10px">Read
                                            More</a>
                                    </div>
                                </div>
                            </a>
                    ...
                    <nav aria-label="...">
                        ...
                        <ul class="pagination">
                            <li class="page-item @if ($ecatalogs->currentPage() <= 1) disabled @endif">
                                <a href="{{ route('e-catalog', ['page' => $ecatalogs->currentPage() - 1]) }}@if (request('search')) &search={{ request('search') }} @endif"
                                    class="page-link">Prev</a>
                            </li>
                            @if ($ecatalogs->lastPage() > 3)
                                    ...
                                    @for ($a = 1; $a <= 3; $a++)
                                        ...
                                        @else
                                            <li class="page-item"><a class="page-link"
                                                    href="{{ route('e-catalog', ['page' => $a]) }}@if (request('search')) &search={{ request('search') }} @endif">{{ $a }}</a>
                                            </li>
                                        @endif
                                    @endfor
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('e-catalog', ['page' => $per5 * 3 + 4]) }}@if (request('search')) &search={{ request('search') }} @endif">{{ $per5 * 3 + 4 }}</a>
                                    </li>
                                ...
                                @else
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('e-catalog', ['page' => $per5 * 3 - 1]) }}@if (request('search')) &search={{ request('search') }} @endif">{{ $per5 * 3 - 1 }}</a>
                                    </li>
                                    ...
                                    @endfor
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('e-catalog', ['page' => $per5 * 3 + 3]) }}@if (request('search')) &search={{ request('search') }} @endif">{{ $per5 * 3 + 3 }}</a>
                                    </li>
                                @endif
                            @else
                                @for ($a = 1; $a <= $ecatalogs->lastPage(); $a++)
                                    @if ($a == $ecatalogs->currentPage())
                                        <li class="page-item active" aria-current="page"><span
                                                class="page-link">{{ $a }}</span></li>
                                    @else
                            ...
                            <li class="page-item @if ($ecatalogs->currentPage() >= $ecatalogs->lastPage()) disabled @endif">
                                <a class="page-link"
                                    href="{{ route('e-catalog', ['page' => $ecatalogs->currentPage() + 1]) }}@if (request('search')) &search={{ request('search') }} @endif">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
