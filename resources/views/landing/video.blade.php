                ...
                <div class="carousel-inner">
                    ...
                    @foreach ($carousel_items as $a => $carousel_item)
                        ...
                        <div class="carousel-item @if ($a == 0) active @endif" data-bs-interval="3000">
                            ...
                            <img src="/uploads/video/image/{{ $carousel_item->banner }}" class="d-block w-100" alt="..."
                            ...
                            <div class="p-3 carousel-caption d-none d-md-block h-100" style="width: 1000px">
                                <div
                                    ...
                                    <div class="mt-4 d-flex justfiy-content-start align-items-start">
                                        <a href="{{ route('video.detail', ['slug' => $carousel_item->slug]) }}"
                    ...
                    @foreach ($carousel_items as $a => $carousel_item)
                        <a href="{{ route('berita.detail', ['slug' => $carousel_item->slug]) }}">
                            <div class="carousel-item @if ($a == 0) active @endif"
                                ...
                                <img src="/uploads/video/image/{{ $carousel_item->banner }}"
        ...
        <div class="container">
            <div class=" d-flex justify-content-center w-100">
                <form action="" method="get">
                    <div class="row">
                        <div class="col">
                            <input type="text" class="form-control" placeholder="Cari Video yang ingin anda tonton"
                            ...
                            @else
                                @if (isset($selected_category))
                                    <h3 class="mt-5" style="font-weight:500;margin:0px;padding:0px;text-align:left;">
                                        Category {{ $selected_category->name }} dari AMANAH Video</h3>
                                @elseif(isset($selected_tag))
                                    <h3 class="mt-5" style="font-weight:500;margin:0px;padding:0px;text-align:left;">Tag
                                        {{ $selected_tag->name }} dari AMANAH Video</h3>
                                @else
                                    <h3 class="mt-5" style="font-weight:500;margin:0px;padding:0px;text-align:left;">Video
                                        Terbaru dari AMANAH</h3>
                                @endif
                    @endif
                </div>
                        ...
                        <ul class="dropdown-menu">
                            @foreach ($categories as $category)
                                <li><a class="dropdown-item"
                                        href="{{ route('video.category', ['slug' => $category->slug]) }}">{{ $category->name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="col-xl-8 ps-2 pe-2">
                    ...
                    @if (strval(request('search')) > 0)
                        @foreach ($search['items'] as $a => $new)
                            ...
                            $content = str_replace('</div>', '', $content);
                            ?>
                            <div class="mb-3 w-100 video-link-hover" data-aos="fade-up" data-aos-delay="50">
                                <a href="{{ route('video.detail', ['slug' => $new->slug]) }}">
                                    ...
                                    <a
                                        href="/berita/detail/{{ $new->slug }}"style=" color:#000000;font-size:16px;font-weight:600;text-align:left;">Tonton
                                        Video ></a>
                                    <hr>
                                </a>
                            </div>
                                ...
                                <nav aria-label="...">
                                    ...
                                    <ul class="pagination">
                                        <li class="page-item @if ($search['currentPage'] <= 1) disabled @endif">
                                            <a href="{{ route('video', ['page' => $search['currentPage'] - 1]) }}&search={{ request('search') }}"
                                        ...
                                        @if ($search['lastPage'] > 3)
                                                ...
                                                @for ($a = 1; $a <= 3; $a++)
                                                    ...
                                                    @else
                                                        <li class="page-item"><a class="page-link"
                                                                href="{{ route('video', ['page' => $a]) }}&search={{ request('search') }}">{{ $a }}</a>
                                                        </li>
                                                    @endif
                                                @endfor
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ route('video', ['page' => $per5 * 3 + 4]) }}&search={{ request('search') }}">{{ $per5 * 3 + 4 }}</a>
                                                </li>
                                            ...
                                            @else
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ route('video', ['page' => $per5 * 3 - 1]) }}&search={{ request('search') }}">{{ $per5 * 3 - 1 }}</a>
                                                </li>
                                                ...
                                                @endfor
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ route('video', ['page' => $per5 * 3 + 3]) }}&search={{ request('search') }}">{{ $per5 * 3 + 3 }}</a>
                                                </li>
                                            @endif
                                        @else
                                        ...
                                        <li class="page-item @if ($search['currentPage'] >= $search['lastPage']) disabled @endif">
                                            <a class="page-link"
                                                href="{{ route('video', ['page' => $search['currentPage'] + 1]) }}&search={{ request('search') }}">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
                                ...
                                <div class="mb-3 w-100 video-link-hover" data-aos="fade-up" data-aos-delay="50">
                                    <a href="{{ route('video.detail', ['slug' => $new->slug]) }}">
                                        ...
                                        <p class="mt-3" style="color:#92929D;font-size:16px;text-align:left;">
                                            <?= $content ?></p>
                                        <a
                                            href="/video/detail/{{ $new->slug }}"style=" color:#000000;font-size:16px;font-weight:600;text-align:left;">Tonton
                                            Video ></a>
                                        <hr>
                                    </a>
                                </div>
                ...
                <div class="p-2 col-xl-4">
                    ...
                    @if ($trendings->first())
                        <div class="mb-3 w-100" data-aos="fade-left" data-aos-delay="100">
                            <iframe style="width:100%; height:230px;"
                                src="https://www.youtube.com/embed/{{ $trendings->first()->video }}"></iframe>
                        </div>
                    @endif
                    {{-- <div class="mb-3 w-100" data-aos="fade-left" data-aos-delay="100">
                    ...
                    @foreach ($trendings as $i => $trending)
                        <div class="mb-3 w-100" data-aos="fade-left" data-aos-delay="100">
                            <a href="{{ route('video.detail', ['slug' => $trending->slug]) }}">
                                ...
                                <img src="/uploads/video/image/{{ $trending->banner }}" alt=""
                ...
                <div class="row">
                    ...
                    <div class="d-flex justify-content-center">
                        ...
                        <nav aria-label="...">
                            ...
                            <ul class="pagination">
                                <li class="page-item @if ($others->currentPage() <= 1) disabled @endif">
                                    <a href="{{ route('video', ['page' => $others->currentPage() - 1]) }}"
                                        class="page-link">Prev</a>
                                </li>
                                @if ($others->currentPage() < 3)
                                    @for ($a = 1; $a <= 3; $a++)
                                        ...
                                        @else
                                            <li class="page-item"><a class="page-link"
                                                    href="{{ route('video', ['page' => $a]) }}">{{ $a }}</a>
                                            </li>
                                        @endif
                                    @endfor
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('video', ['page' => $per5 * 3 + 4]) }}">{{ $per5 * 3 + 4 }}</a>
                                    </li>
                                @elseif($others->currentPage() > $others->lastPage() - 3)
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('video', ['page' => $per5 * 3 - 4]) }}">{{ $per5 * 3 - 4 }}</a>
                                ...
                                @else
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('video', ['page' => $per5 * 3 - 1]) }}">{{ $per5 * 3 - 1 }}</a>
                                    </li>
                                    @for ($a = $per5 * 3; $a < $per5 * 3 + 3; $a++)
                                        @if ($a == $others->currentPage())
                                            <li class="page-item active" aria-current="page"><span
                                    ...
                                    @endfor
                                    <li class="page-item"><a class="page-link"
                                            href="{{ route('video', ['page' => $per5 * 3 + 3]) }}">{{ $per5 * 3 + 3 }}</a>
                                    </li>
                                @endif
                                <li class="page-item @if ($others->currentPage() >= $others->lastPage()) disabled @endif">
                                    <a class="page-link"
                                        href="{{ route('video', ['page' => $others->currentPage() + 1]) }}">Next</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
