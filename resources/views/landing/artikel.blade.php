        ...
        <div class="container">
            <div class=" d-flex justify-content-center w-100">
                <form action="" method="get">
                    <div class="row">
                        <div class="col">
                            <input type="text" class="form-control" placeholder="Cari Artikel yang ingin anda baca"
                        ...
                        <ul class="dropdown-menu">
                            @foreach ($categories as $category)
                                <li><a class="dropdown-item"
                                        href="{{ route('artikel.category', ['slug' => $category->slug]) }}">{{ $category->name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="col-xl-8 ps-2 pe-2">
                    ...
                    @if (strval(request('search')) > 0)
                        @foreach ($search['items'] as $a => $new)
                            ...
                            $content = str_replace('</div>', '', $content);
                            ?>
                            <div class="mb-3 w-100 article-link-hover" data-aos="fade-up" data-aos-delay="50">
                                ...
                                <nav aria-label="...">
                                    ...
                                    <ul class="pagination">
                                        <li class="page-item @if ($search['currentPage'] <= 1) disabled @endif">
                                            <a href="{{ route('artikel', ['page' => $search['currentPage'] - 1]) }}&search={{ request('search') }}"
                                        ...
                                        @if ($search['lastPage'] > 3)
                                                ...
                                                @for ($a = 1; $a <= 3; $a++)
                                                    ...
                                                    @else
                                                        <li class="page-item"><a class="page-link"
                                                                href="{{ route('artikel', ['page' => $a]) }}&search={{ request('search') }}">{{ $a }}</a>
                                                        </li>
                                                    @endif
                                                @endfor
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ route('artikel', ['page' => $per5 * 3 + 4]) }}&search={{ request('search') }}">{{ $per5 * 3 + 4 }}</a>
                                                </li>
                                            ...
                                            @else
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ route('artikel', ['page' => $per5 * 3 - 1]) }}&search={{ request('search') }}">{{ $per5 * 3 - 1 }}</a>
                                                </li>
                                                ...
                                                @endfor
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ route('artikel', ['page' => $per5 * 3 + 3]) }}&search={{ request('search') }}">{{ $per5 * 3 + 3 }}</a>
                                                </li>
                                            @endif
                                        @else
                                        ...
                                        <li class="page-item @if ($search['currentPage'] >= $search['lastPage']) disabled @endif">
                                            <a class="page-link"
                                                href="{{ route('artikel', ['page' => $search['currentPage'] + 1]) }}&search={{ request('search') }}">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
                    ...
                    @foreach ($trendings as $i => $trending)
                        <div class="mb-3 w-100" data-aos="fade-left" data-aos-delay="100">
                            <a href="{{ route('artikel.detail', ['slug' => $trending->slug]) }}">
                                ...
                                <img src="/uploads/artikel/image/{{ $trending->banner }}" alt=""
                                    style="max-height:300px;width: 100%">
                                <h4 class="mt-3 mb-4" style="font-weight:600;">{{ $trending->title }}</h4>
                                <a href="/artikel/detail/{{ $trending->slug }}">Baca Artikel ></a>
                                <hr>
                            </a>
                        </div>
                    @endforeach
